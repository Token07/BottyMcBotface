...
export type BottyCommandInteraction = (interaction: Discord.CommandInteraction | Discord.AutocompleteInteraction, admin?: false | true) => void | any;
...
export default class InteractionManager {
    ...
    public async addSlashCommand(command: Discord.RESTPostAPIChatInputApplicationCommandsJSONBody | Discord.RESTPatchAPIApplicationCommandJSONBody, global: boolean, adminOnly: boolean, handler: BottyCommandInteraction) {
        ...
        if (this.initialized === "failed") return console.error("InteractionManager: tried to add a command but intialization has failed");
    ...
    public onInteraction(interaction: Discord.BaseInteraction) {
        ...
        if (interaction instanceof Discord.CommandInteraction) {
            ...
            // if (handler === undefined) return interaction.reply({content: "There is no function to handle this command (?!)", ephemeral: true})
            // initialize() probably hasn't finished if its undefined
            if (handler === undefined) {
                const possibleInteraction = this.findInteractionByName(interaction.commandName);
                console.warn("InteractionManager: attempting to determine correct interaction by name instead of id");
                try {
    ...
    private findInteractionByName(name: string, guild?: string) {
